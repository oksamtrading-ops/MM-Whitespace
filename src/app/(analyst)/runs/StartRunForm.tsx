...
import { CONFIRM_ABOVE, SCOPE_COPY, type Estimate, type Scope } from "../../../lib/enrich/scope.ts";

export type ScopeOffer = { scope: Scope; estimate: Estimate; allowed: boolean; why?: string };
...
export default function StartRunForm({ periodId, offers, defaultBudgetUsd, mode }: {
  ...
  const [scope, setScope] = useState<Scope>(offers.find((o) => o.allowed)?.scope ?? "unresearched");
  const [budget, setBudget] = useState(String(defaultBudgetUsd));
  const chosen = offers.find((o) => o.scope === scope) ?? offers[0];
  ...
  return (
    <form action={action} className="commit settings">
      ...
      <fieldset className="field">
        <legend>Scope</legend>
        {offers.map((o) => (
          <p key={o.scope} style={{ margin: "6px 0" }}>
            <label>
              <input type="radio" name="scope" value={o.scope} checked={scope === o.scope}
                     disabled={!o.allowed} onChange={() => setScope(o.scope)} />{" "}
              {SCOPE_COPY[o.scope].label} — <b>{o.estimate.count}</b>
              {o.estimate.count === 1 ? " company" : " companies"}
            </label>
            <span className="hint">{o.allowed ? SCOPE_COPY[o.scope].detail : o.why}</span>
          </p>
        ))}
      </fieldset>
      ...
      <p className="meta" style={{ marginTop: 12 }}>
        ...
        {mode === "replay"
          ...
          : "Live mode sends prompts to the model vendor within the budget above."}
      </p>
    </form>
  );
}
