...
test("a stored stamp is read as UTC, not as local time", () => {
...
test("a bare date is a calendar date, so it keeps its own day", () => {
...
test("days since is never negative", () => {
...
test("recent sign-ins are worded, older ones counted", () => {
  assert.equal(agoLabel(0), "today");
  assert.equal(agoLabel(1), "yesterday");
  assert.equal(agoLabel(97), "97d ago");
});

test("period labels and thresholds are shortened for display", () => {
