...
test("the canonical Postgres migrations apply, and the policy file is skipped", async () => {
  const db = fresh();
  const r = await applySchema(new DatabaseSync(":memory:"));
  ...
  const tables =(await db.all("select name from sqlite_master where type='table'")).map((t: any) => t.name);
...
test("no CLIENT information is stored, and internal-but-not-client data is separated", async () => {
  const db = fresh();
  // Client information is out of scope entirely: there is no column for it.
  const cols =(await db.all("pragma table_info(companies)")).map((c: any) => c.name);
  assert.ok(!cols.includes("deloitte_tax_client"),
    "the POC stores no Deloitte client information");

  await commitPeriod(db, payload([company({ dtt_market: "Ontario" })]));
  ...
  const internal = await db.all(`select distinct v.field_key from company_period_field_values v
       join field_catalog f on f.key = v.field_key
      where f.classification = 'deloitte_internal'
        and v.value is not null and v.value != 'null'`) as Array<{ field_key: string }>;
...
test("committing twice is idempotent", async () => {
  const db = fresh();
  const a = await commitPeriod(db, payload([company(), company({ ticker: "STHC", name: "Southco Ltd." })]));
  const b = await commitPeriod(db, payload([company(), company({ ticker: "STHC", name: "Southco Ltd." })]));
  ...
  for (const table of ["companies", "company_period_facts", "company_period_field_values",
    ...
    const n = await db.get(`select count(*) n from ${table}`) as { n: number };
    assert.ok(n.n > 0, `${table} should hold rows`);
  }
  const dupes = await db.get("select count(*) n from companies") as { n: number };
  ...
  const idents = await db.get("select count(*) n from company_identifiers") as { n: number };
  assert.equal(idents.n,a.identifiers, "a re-commit must not add identifier rows");
});

test("an identifier with no exchange does not re-insert on every import", async () => {
  ...
  await commitPeriod(db, payload([withId]));
  const first = await db.get("select count(*) n from company_identifiers where scheme = 'sp_entity_id'") as { n: number };
  await commitPeriod(db, payload([withId]));
  const second = await db.get("select count(*) n from company_identifiers where scheme = 'sp_entity_id'") as { n: number };
  assert.equal(first.n, 1);
  assert.equal(second.n, 1, "a re-import must not duplicate an exchange-less identifier");
});

test("a manual override survives a re-import; the database enforces it, not the caller", async () => {
  const db = fresh();
  const r = await commitPeriod(db, payload([company()]));
  const co = await db.get("select id from companies limit 1") as { id: string };db.run(`update company_period_field_values set value = ?, source = 'manual_override'
      where period_id = ? and company_id = ? and field_key = 'auditor'`, JSON.stringify("KPMG"),r.periodId, co.id);

  // Re-import claims Deloitte again. The conditional upsert must refuse.
  await commitPeriod(db, payload([company({ auditor: "Deloitte" })]));
  const row = await db.get(`select value, source from company_period_field_values
      where period_id = ? and company_id = ? and field_key = 'auditor'`,r.periodId, co.id) as { value: string; source: string };
  assert.equal(row.source, "manual_override");
  assert.equal(JSON.parse(row.value), "KPMG");
});

test("an extract value IS updated by a re-import", async () => {
  const db = fresh();
  const r = await commitPeriod(db, payload([company({ market_cap: 1 })]));
  const co = await db.get("select id from companies limit 1") as { id: string };
  await commitPeriod(db, payload([company({ market_cap: 2 })]));
  const row = await db.get(`select value, source from company_period_field_values
      where period_id = ? and company_id = ? and field_key = 'market_cap_cad'`,r.periodId, co.id) as { value: string; source: string };
  assert.equal(row.source, "extract");
  assert.equal(JSON.parse(row.value), 2);
});

test("a frozen value is never overwritten, even when its source is extract", async () => {
  const db = fresh();
  const r = await commitPeriod(db, payload([company({ market_cap: 1 })]));
  const co = await db.get("select id from companies limit 1") as { id: string };db.run(`update company_period_field_values set frozen_at = (datetime('now'))
      where period_id = ? and company_id = ? and field_key = 'market_cap_cad'`,r.periodId, co.id);
  await commitPeriod(db, payload([company({ market_cap: 999 })]));
  const row = await db.get(`select value from company_period_field_values
      where period_id = ? and company_id = ? and field_key = 'market_cap_cad'`,r.periodId, co.id) as { value: string };
  assert.equal(JSON.parse(row.value), 1, "publish freezes a value; a re-import must not move it");
});

test("an analyst-retained region is recorded as asserted, not as absent", async () => {
  const db = fresh();
  const r = await commitPeriod(db, payload([company({
    regions: { CANADA: ["NT"] },
  ...
  const fact = await db.get(`select assertion from company_period_facts
      where period_id = ? and field_key = 'property_regions'`,r.periodId) as { assertion: string };
...
test("a venture graduation closes one identifier interval and opens another on the same company", async () => {
  const db = fresh();
  await commitPeriod(db, payload([company({ ticker: "GRAD", exchange: "TSXV" })], "2026-05-31"));
  const before = await db.get("select count(*) n from companies") as { n: number };

  await commitPeriod(db, payload([company({ ticker: "GRAD", exchange: "TSX" })], "2026-08-31"));
  const after = await db.get("select count(*) n from companies") as { n: number };
  assert.equal(after.n, before.n, "a graduation is not a drop-out plus a new entrant");

  const ids = await db.all(`select exchange, valid_to from company_identifiers
      where scheme = 'root_ticker' order by valid_from`) as Array<{ exchange: string; valid_to: string | null }>;
...
test("a published period cannot be re-committed, and an earlier period is refused", async () => {
  const db = fresh();
  const r = await commitPeriod(db, payload([company()], "2026-05-31"));
  await db.run("update periods set status = 'published' where id = ?",r.periodId);

  await assert.rejects(() => commitPeriod(db, payload([company()], "2026-05-31")),
    /already published/);
  await assert.rejects(() => commitPeriod(db, payload([company()], "2026-02-28")),
    /predates the published period/);
});

test("the trace is stored rule by rule, not as a sentence", async () => {
  const db = fresh();
  const r = await commitPeriod(db, payload([company({ stages: [], stage_evidence: "none" })]));
  const trace = await db.all(`select ord, rule_id, matched from tier_traces where period_id = ? order by ord`,r.periodId) as Array<{ rule_id: string; matched: number }>;
  assert.equal(trace.length, 1);
  assert.equal(trace[0].rule_id, "guard_no_stage_evidence");
  assert.equal(trace[0].matched, 1);
  const tier = await db.get("select tier, status from tiers where period_id = ?",r.periodId) as { tier: number | null; status: string };
  assert.equal(tier.tier, null);
  assert.equal(tier.status, "unclassified_no_stage_evidence");
});

test("a failed commit rolls back entirely", async () => {
  const db = fresh();
  await assert.rejects(() => commitPeriod(db, payload([company(), company({ stages: ["bogus" as any] })])));
  const n = await db.get("select count(*) n from companies") as { n: number };
  assert.equal(n.n, 0, "a rejected stage must not leave a half-written period behind");
});

test("the dialect refuses what it cannot faithfully translate", async () => {
