...
function deps(over: Partial<FetchDeps> & { responses?: Record<string, HttpResponse> } = {}): FetchDeps {
...
test("private, loopback and link-local address space is blocked", () => {
  for (const ip of [
...
test("a hostname that resolves into private space is refused after resolution", async () => {
  ...
  await assert.rejects(
    fetchDocument("https://northco.invalid/aif.html", ALLOW,
      deps({ resolve: async () => ["169.254.169.254"] })),
...
test("a redirect into private space is refused exactly as a direct request is", async () => {
  ...
  let call = 0;
  await assert.rejects(
    fetchDocument("https://northco.invalid/doc", ALLOW, deps({
      responses,
...
test("a redirect off the allowlist is refused", async () => {
  ...
  await assert.rejects(
    fetchDocument("https://northco.invalid/doc", ALLOW, deps({ responses })),
    (e: FetchRefused) => e.code === "host_not_allowed");
});

// -------------------------------------------------------------- allowlist

test("the allowlist matches on label boundaries, not on suffix", () => {
...
test("a domain the model proposed is not fetchable on that basis", async () => {
  await assert.rejects(
    fetchDocument("https://model-suggested.invalid/fees.pdf", ALLOW, deps()),
    (e: FetchRefused) => {
...
test("type is sniffed from the bytes, never from the declared type", () => {
...
test("content declared as PDF whose bytes are not a PDF is refused", async () => {
  await assert.rejects(
    fetchDocument("https://northco.invalid/fees.pdf", ALLOW, deps({
      responses: {
...
test("a PDF with no extractor supplied is refused rather than guessed at", async () => {
  await assert.rejects(
    fetchDocument("https://northco.invalid/aif.pdf", ALLOW, deps({
      responses: {
...
test("scripts and styles never reach the stored text", () => {
...
test("a fetched document is keyed by hash and stores text, never the file", async () => {
  ...
  const doc = await fetchDocument("https://northco.invalid/circular.html", ALLOW,
    deps({ responses: { "https://northco.invalid/circular.html": ok(html) } }));
...
test("the same bytes always produce the same hash, so the cache is stable", async () => {
  ...
  const one = await fetchDocument("https://northco.invalid/a.html", ALLOW,
    deps({ responses: { "https://northco.invalid/a.html": ok(html) } }));
  const two = await fetchDocument("https://royalco.invalid/b.html", ALLOW,
    deps({ responses: { "https://royalco.invalid/b.html": ok(html) } }));
...
test("a scanned filing is marked as having no text layer before it is used", async () => {
  ...
  const doc = await fetchDocument("https://northco.invalid/scan.pdf", ALLOW, deps({
    extractors,
    responses: {
...
test("an oversized page count is refused", async () => {
  ...
  await assert.rejects(
    fetchDocument("https://northco.invalid/huge.pdf", ALLOW, deps({
      extractors,
      responses: {
...
test("only http and https are fetchable", async () => {
  for (const url of ["file:///etc/passwd", "ftp://northco.invalid/x", "gopher://x/"]) {
    await assert.rejects(fetchDocument(url, ALLOW, deps()),
      (e: FetchRefused) => e.code === "scheme" || e.code === "malformed_url");
  }
});

test("source tier is computed from the resolved host, not asked of the model", () => {
...
test("the allowlist is derived from the extract, and page titles are ignored", async () => {
  ...
  const put = (companyId: string, value: string) =>
    db.prepare(
      `insert into company_period_field_values
       ...
       values (?, ?, 'website', ?, 'extract', 'asserted')`,
    ).run(periodId, companyId, JSON.stringify(value));
  ...
  const allow = allowlistFromPeriod(db, periodId);
  assert.equal(isAllowedHost("northco.invalid", allow), true);
  assert.equal(isAllowedHost("royalco.invalid", allow), false,
  ...
  const doc = await fetchDocument("https://northco.invalid/x.html", allow,
    deps({ responses: { "https://northco.invalid/x.html": ok("<html><body>Audit fees 412</body></html>") } }));
  const hash = storeDocument(db, doc);
  const stored = db.prepare(
    "select text_content, has_text_layer, source_tier from documents where content_hash = ?",
  ).get(hash) as any;
