...
const period = db.prepare("select id, label, status from periods order by rowid desc limit 1").get();
console.log(`period: ${period.label}  (${period.status})\n`);

const gate = evaluateGate(db, period.id);
...
if (doPublish) {
  try {
    const r = publishPeriod(db, period.id, {
      overrideReason,
    ...
    const snap = readPublished(db, period.id);
